...
func TestSuperAdminAuth(t *testing.T) {
	_, _, err := service.AdminLogin(Ts.URL+"/api", SuperAdmin.Email, SuperAdmin.Password)
...
func TestCommonAdminAuth(t *testing.T) {
	...
	_, _, err := service.AdminRegisterAndLogin(Ts.URL+"/api", userInfo.Email, userInfo.Password, userInfo.Name)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAdminLoginWrong(t *testing.T) {
	...
	ok, _, err := service.AdminLogin(Ts.URL+"/api", userInfo.Email, userInfo.Password+"123")
...
func TestAdminRegisterEmailUsed(t *testing.T) {
	...
	ok, _, err := service.AdminRegisterAndLogin(Ts.URL+"/api", userInfo.Email, userInfo.Password, userInfo.Name)
...
func TestAdminInvitationCodeFrequence(t *testing.T) {
	...
	_, err := service.AdminInvitationCode(Ts.URL+"/api", email)
	if err != nil {
		t.Fatal(err)
	}
	ok, err := service.AdminInvitationCode(Ts.URL+"/api", email)
...
func TestAdminRegisterWrongCode(t *testing.T) {
	...
	_, err := service.AdminInvitationCode(Ts.URL+"/api", userInfo.Email)
	if err != nil {
		t.Fatal(err)
	}
	...
	ok, err := service.AdminRegister(Ts.URL+"/api", userInfo.Email, userInfo.Password, userInfo.Name, invitationCode.Code+"123")
...
func TestAdminSubToken(t *testing.T) {
	ok, token, _ := service.AdminLogin(Ts.URL+"/api", SuperAdmin.Email, SuperAdmin.Password)
	if !ok {
		t.Fatal("Test Admin SubToken, SuperAdmin Login Fail")
	}
	_, err := service.ProjectInsert(Ts.URL+"/api", token, "test_storage_upload"+util.RandString(6), "^B\\d{8}-.{2,4}-.{2}\\d$", []string{"doc", "docx"}, []string{"label1_" + util.RandString(4)})
	if err != nil {
		t.Fatal(err)
	}
	...
	ok, subToken := service.AdminSubToken(Ts.URL+"/api", token, 10, 5)
	if !ok {
		t.Fatal("Test Admin SubToken, Request SubToken Fail")
	}

	ok, _, err = service.ProjectOwn(Ts.URL+"/api", subToken)
	if ok {
		log.Println(err)
	...
	_, err = service.StorageDownload(Ts.URL+"/api", subToken, projectID)
	if err != nil {
		t.Fatal(err)
	}
}
